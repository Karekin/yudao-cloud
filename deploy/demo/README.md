...
2. Restore or clone a complete Yudao Demo database, apply and verify CloudMold
   V01-V12 in order, and record the backup ID and schema checksum. The repository
   base SQL alone is not a complete Mall/Member/WMS schema and must not be used
   as a fresh release database.
...
checks MySQL 8, the complete Yudao module baseline, every table created by the
ordered CloudMold migrations, and representative migration columns, indexes,
and constraints:
