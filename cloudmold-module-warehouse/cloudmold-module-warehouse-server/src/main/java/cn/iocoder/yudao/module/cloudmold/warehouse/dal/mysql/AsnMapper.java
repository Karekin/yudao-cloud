...
public interface AsnMapper extends BaseMapperX<AsnDO> {
    @Select("SELECT * FROM cloudmold_asn WHERE tenant_id=#{tenantId} AND asn_id=#{asnId} FOR UPDATE")
    AsnDO selectForUpdate(@Param("tenantId") Long tenantId, @Param("asnId") String asnId);

    @Select("SELECT * FROM cloudmold_asn WHERE tenant_id=#{tenantId} AND source_business_type=#{sourceBusinessType} AND source_business_ref=#{sourceBusinessRef}")
    AsnDO selectBySourceRef(@Param("tenantId") Long tenantId, @Param("sourceBusinessType") String sourceBusinessType,
                            @Param("sourceBusinessRef") String sourceBusinessRef);

    @Update("UPDATE cloudmold_asn SET status=#{status},version=version+1,updated_at=#{now} "
