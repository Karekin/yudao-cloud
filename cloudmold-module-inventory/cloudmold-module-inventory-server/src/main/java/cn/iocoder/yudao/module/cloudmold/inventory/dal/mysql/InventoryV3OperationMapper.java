...
import cn.iocoder.yudao.module.cloudmold.inventory.dal.dataobject.InventoryV3OperationDO;
import org.apache.ibatis.annotations.*;

import java.time.LocalDateTime;

@Mapper
public interface InventoryV3OperationMapper extends BaseMapperX<InventoryV3OperationDO> {
    @Insert("""
            INSERT INTO cloudmold_inventory_operation_v3
    ...
    int insertOrResolve(@Param("tenantId") Long tenantId, @Param("idempotencyKey") String idempotencyKey,
                        @Param("sourceEventId") String sourceEventId, @Param("commandType") String commandType,
    ...
    @Select("SELECT * FROM cloudmold_inventory_operation_v3 WHERE tenant_id=#{tenantId} AND operation_id=#{operationId} FOR UPDATE")
    InventoryV3OperationDO selectForUpdate(@Param("operationId") Long operationId, @Param("tenantId") Long tenantId);

    @Update("""
            UPDATE cloudmold_inventory_operation_v3
            SET status=10,ledger_transaction_id=#{ledgerTransactionId},result_json=#{resultJson},updated_at=#{now}
