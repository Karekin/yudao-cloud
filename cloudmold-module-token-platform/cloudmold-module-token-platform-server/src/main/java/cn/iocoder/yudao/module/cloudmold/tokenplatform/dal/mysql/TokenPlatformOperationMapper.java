...
import cn.iocoder.yudao.module.cloudmold.tokenplatform.dal.dataobject.TokenPlatformOperationDO;
import org.apache.ibatis.annotations.*;

import java.time.LocalDateTime;

@Mapper
public interface TokenPlatformOperationMapper extends BaseMapperX<TokenPlatformOperationDO> {
    @Insert("""
        INSERT INTO cloudmold_token_platform_operation
    ...
    @Select("SELECT * FROM cloudmold_token_platform_operation WHERE operation_id=#{operationId} AND tenant_id=#{tenantId} FOR UPDATE")
    TokenPlatformOperationDO selectForUpdate(@Param("operationId") Long operationId,
                                             @Param("tenantId") Long tenantId);

    @Update("""
        UPDATE cloudmold_token_platform_operation
