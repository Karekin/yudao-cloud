...
import cn.iocoder.yudao.module.cloudmold.promotion.dal.dataobject.PromotionOperationDO;
import org.apache.ibatis.annotations.*;

import java.time.LocalDateTime;

@Mapper
public interface PromotionOperationMapper extends BaseMapperX<PromotionOperationDO> {
    @Insert("""
        INSERT INTO cloudmold_promotion_operation
    ...
    @Select("SELECT * FROM cloudmold_promotion_operation WHERE operation_id=#{operationId} AND tenant_id=#{tenantId} FOR UPDATE")
    PromotionOperationDO selectForUpdate(@Param("operationId") Long operationId, @Param("tenantId") Long tenantId);

    @Update("""
        UPDATE cloudmold_promotion_operation
