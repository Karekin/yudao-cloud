-- Derive exact legacy Trade benefit-component-to-item evidence from the immutable V41 item denominator.
-- This view does not duplicate facts or authorize import. It makes header/item differences queryable.
...
WITH eligible_item AS (
  ...
  SELECT tenant_id,migration_run_id,candidate_id,legacy_order_id,item_evidence_id,is_deleted,
         'GENERIC_DISCOUNT' component_type,generic_discount_amount_minor amount_minor
  FROM eligible_item
  UNION ALL
  SELECT tenant_id,migration_run_id,candidate_id,legacy_order_id,item_evidence_id,is_deleted,
         'COUPON',coupon_amount_minor
  FROM eligible_item
  UNION ALL
  SELECT tenant_id,migration_run_id,candidate_id,legacy_order_id,item_evidence_id,is_deleted,
         'POINT',point_amount_minor
  FROM eligible_item
  UNION ALL
  SELECT tenant_id,migration_run_id,candidate_id,legacy_order_id,item_evidence_id,is_deleted,
         'VIP',vip_amount_minor
  FROM eligible_item
), item_rollup AS (
...
SELECT
  ...
  SHA2(CONCAT_WS('|',component_keys.candidate_id,component_keys.component_type,
      ...
      COALESCE(item_rollup.item_component_row_count,0),
      COALESCE(item_rollup.item_component_amount_minor,0),
