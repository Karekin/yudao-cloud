...
import cn.iocoder.yudao.module.cloudmold.fulfillment.dal.dataobject.FulfillmentOperationDO;
import org.apache.ibatis.annotations.*;

import java.time.LocalDateTime;

@Mapper
public interface FulfillmentOperationMapper extends BaseMapperX<FulfillmentOperationDO> {
    @Insert("""
            INSERT INTO cloudmold_fulfillment_operation
    ...
    @Select("""
            SELECT operation_id,tenant_id,idempotency_key,command_type,request_hash,attempt_token,status,
                   fulfillment_id,result_json,created_at,updated_at
            FROM cloudmold_fulfillment_operation
            WHERE operation_id=#{operationId} AND tenant_id=#{tenantId}
            FOR UPDATE
            """)
    FulfillmentOperationDO selectForUpdate(@Param("operationId") Long operationId,
                                           @Param("tenantId") Long tenantId);

    @Update("""
            UPDATE cloudmold_fulfillment_operation
            SET status=10,fulfillment_id=#{fulfillmentId},result_json=CAST(#{resultJson} AS JSON),updated_at=#{now}
    ...
    int markSucceeded(@Param("operationId") Long operationId, @Param("tenantId") Long tenantId,
                      @Param("fulfillmentId") String fulfillmentId, @Param("resultJson") String resultJson,
