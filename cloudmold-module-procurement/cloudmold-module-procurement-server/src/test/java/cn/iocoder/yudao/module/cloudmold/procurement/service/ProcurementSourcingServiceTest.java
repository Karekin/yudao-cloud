...
class ProcurementSourcingServiceTest {
    ...
    @Test void independentCheckerApprovesAndCreatesImmutableSnapshot(){
        ...
        when(procurement.selectPurchaseRequisitionById(31L,"pr-1")).thenReturn(new PurchaseRequisition().setRequisitionId("pr-1").setVersion(1L).setLegalEntityId("legal-entity-01").setTaxCalculationPolicyCode("STANDARD_V1").setRoundingPolicyCode("HALF_UP"));
        when(procurement.selectPurchaseRequisitionLines(31L,"pr-1")).thenReturn(List.of(new PurchaseRequisitionLine().setLineId("prl-1").setCanonicalSkuId("sku-1").setRequestedQuantity(new BigDecimal("8")).setUomCode("EA").setValuationPolicyId("valuation-policy-01").setValuationPolicyVersion("v1").setValuationPolicyHash("d".repeat(64))));
        when(procurement.selectPurchaseRequisitionSchedules(31L,"pr-1")).thenReturn(List.of(new PurchaseRequisitionDeliverySchedule().setScheduleId("prs-1").setLineId("prl-1").setCanonicalWarehouseId("wh-1").setScheduledQuantity(new BigDecimal("8")).setRequiredDeliveryDate(LocalDate.of(2026,8,20))));
        when(sourcing.selectLines(31L,"rfq-1")).thenReturn(List.of(line().setRequisitionLineId("prl-1")));
        when(sourcing.selectSchedules(31L,"rfq-1")).thenReturn(List.of(schedule("rfqs-1",new BigDecimal("8")).setRequisitionScheduleId("prs-1")));
    ...
    private static SourcingLine line(){return new SourcingLine().setSourcingLineId("rfql-1").setRequisitionLineId("prl-1").setCanonicalSkuId("sku-1").setRequestedQuantity(new BigDecimal("20")).setUomCode("EA");}
    private static SourcingSchedule schedule(String id,BigDecimal qty){return new SourcingSchedule().setSourcingScheduleId(id).setSourcingLineId("rfql-1").setRequisitionScheduleId(id.equals("rfqs-1")?"prs-1":"prs-2").setCanonicalWarehouseId(id.equals("rfqs-1")?"wh-1":"wh-2").setRequestedQuantity(qty);}
