...
public interface ProcurementSourcingMapper {
    ...
    @Insert("INSERT INTO cloudmold_procurement_award_snapshot(snapshot_id,tenant_id,award_id,award_version,event_id,event_version,requisition_id,requisition_version,legal_entity_id,tax_calculation_policy_code,rounding_policy_code,status,decision_reason_code,approved_by_principal_id,approved_at,created_at) VALUES(#{snapshotId},#{tenantId},#{awardId},#{awardVersion},#{eventId},#{eventVersion},#{requisitionId},#{requisitionVersion},#{legalEntityId},#{taxCalculationPolicyCode},#{roundingPolicyCode},#{status},#{decisionReasonCode},#{approvedByPrincipalId},#{approvedAt},#{createdAt})") int insertAwardSnapshot(AwardSnapshot value);
    @Insert("""
      <script>INSERT INTO cloudmold_procurement_award_snapshot_line(snapshot_line_id,tenant_id,snapshot_id,award_id,award_line_id,line_number,sourcing_line_id,sourcing_schedule_id,quotation_revision_line_id,quotation_revision_schedule_id,supplier_id,canonical_sku_id,canonical_warehouse_id,awarded_quantity,uom_code,currency_code,unit_net_price_minor,tax_code,tax_rate_bps,promised_delivery_date,line_net_amount_minor,line_tax_amount_minor,line_gross_amount_minor,policy_id,policy_version,evaluation_weighted_score_bps,evaluation_summary_sha256,reviewer_evidence_sha256,requisition_line_id,requisition_schedule_id,valuation_policy_id,valuation_policy_version,valuation_policy_hash) VALUES
      <foreach collection="values" item="v" separator=",">(#{v.snapshotLineId},#{v.tenantId},#{v.snapshotId},#{v.awardId},#{v.awardLineId},#{v.lineNumber},#{v.sourcingLineId},#{v.sourcingScheduleId},#{v.quotationRevisionLineId},#{v.quotationRevisionScheduleId},#{v.supplierId},#{v.canonicalSkuId},#{v.canonicalWarehouseId},#{v.awardedQuantity},#{v.uomCode},#{v.currencyCode},#{v.unitNetPriceMinor},#{v.taxCode},#{v.taxRateBps},#{v.promisedDeliveryDate},#{v.lineNetAmountMinor},#{v.lineTaxAmountMinor},#{v.lineGrossAmountMinor},#{v.policyId},#{v.policyVersion},#{v.evaluationWeightedScoreBps},#{v.evaluationSummarySha256},#{v.reviewerEvidenceSha256},#{v.requisitionLineId},#{v.requisitionScheduleId},#{v.valuationPolicyId},#{v.valuationPolicyVersion},#{v.valuationPolicyHash})</foreach></script>
      """) int insertAwardSnapshotLines(@Param("values") List<AwardSnapshotLine> values);
    @Select("SELECT snapshot_id,tenant_id,award_id,award_version,event_id,event_version,requisition_id,requisition_version,legal_entity_id,tax_calculation_policy_code,rounding_policy_code,status,decision_reason_code,approved_by_principal_id,approved_at,created_at FROM cloudmold_procurement_award_snapshot WHERE tenant_id=#{tenantId} AND award_id=#{awardId} AND award_version=#{awardVersion} FOR UPDATE") AwardSnapshot selectAwardSnapshotForUpdate(@Param("tenantId") Long tenantId,@Param("awardId") String awardId,@Param("awardVersion") Long awardVersion);
    @Select("SELECT snapshot_line_id,tenant_id,snapshot_id,award_id,award_line_id,line_number,sourcing_line_id,sourcing_schedule_id,quotation_revision_line_id,quotation_revision_schedule_id,supplier_id,canonical_sku_id,canonical_warehouse_id,awarded_quantity,uom_code,currency_code,unit_net_price_minor,tax_code,tax_rate_bps,promised_delivery_date,line_net_amount_minor,line_tax_amount_minor,line_gross_amount_minor,policy_id,policy_version,evaluation_weighted_score_bps,evaluation_summary_sha256,reviewer_evidence_sha256,requisition_line_id,requisition_schedule_id,valuation_policy_id,valuation_policy_version,valuation_policy_hash FROM cloudmold_procurement_award_snapshot_line WHERE tenant_id=#{tenantId} AND snapshot_id=#{snapshotId} ORDER BY line_number,snapshot_line_id") List<AwardSnapshotLine> selectAwardSnapshotLines(@Param("tenantId") Long tenantId,@Param("snapshotId") String snapshotId);
