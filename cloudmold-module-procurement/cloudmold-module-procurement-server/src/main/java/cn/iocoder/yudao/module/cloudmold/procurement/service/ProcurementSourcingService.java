...
public class ProcurementSourcingService implements SourcingCommandApi {
    ...
    private Outcome approveAward(Long t,Long op,SourcingCommand.AwardTransitionDefinition d,String actor,LocalDateTime now) {
        ...
        PurchaseRequisition requisition=nn(procurementMapper.selectPurchaseRequisitionById(t,e.getRequisitionId()),"purchase requisition not found");
        req(Objects.equals(requisition.getVersion(),e.getRequisitionVersion()),"award approval requires the exact approved purchase requisition version");
        ref(requisition.getLegalEntityId(),"legalEntityId",128); code(requisition.getTaxCalculationPolicyCode(),"taxCalculationPolicyCode"); code(requisition.getRoundingPolicyCode(),"roundingPolicyCode");
        Map<String,PurchaseRequisitionLine> requisitionLines=index(procurementMapper.selectPurchaseRequisitionLines(t,requisition.getRequisitionId()),PurchaseRequisitionLine::getLineId);
        Map<String,PurchaseRequisitionDeliverySchedule> requisitionSchedules=index(procurementMapper.selectPurchaseRequisitionSchedules(t,requisition.getRequisitionId()),PurchaseRequisitionDeliverySchedule::getScheduleId);
        Map<String,SourcingLine> sourcingLines=index(mapper.selectLines(t,e.getEventId()),SourcingLine::getSourcingLineId);
        Map<String,SourcingSchedule> sourcingSchedules=index(mapper.selectSchedules(t,e.getEventId()),SourcingSchedule::getSourcingScheduleId);
        AwardSnapshot snapshot=new AwardSnapshot().setSnapshotId(snapshotId).setTenantId(t).setAwardId(a.getAwardId()).setAwardVersion(a.getVersion()).setEventId(e.getEventId()).setEventVersion(eventVersion)
                .setRequisitionId(requisition.getRequisitionId()).setRequisitionVersion(requisition.getVersion()).setLegalEntityId(requisition.getLegalEntityId())
                .setTaxCalculationPolicyCode(requisition.getTaxCalculationPolicyCode()).setRoundingPolicyCode(requisition.getRoundingPolicyCode())
        ...
        List<AwardSnapshotLine> snapshots=new ArrayList<>();
        for(AwardLine line:lines){
            SourcingLine sourcingLine=nn(sourcingLines.get(line.getSourcingLineId()),"award snapshot requires sourcing line");
            SourcingSchedule sourcingSchedule=nn(sourcingSchedules.get(line.getSourcingScheduleId()),"award snapshot requires sourcing schedule");
            PurchaseRequisitionLine requisitionLine=nn(requisitionLines.get(sourcingLine.getRequisitionLineId()),"award snapshot requires requisition line");
            PurchaseRequisitionDeliverySchedule requisitionSchedule=nn(requisitionSchedules.get(sourcingSchedule.getRequisitionScheduleId()),"award snapshot requires requisition schedule");
            AwardSnapshotLine s=new AwardSnapshotLine();
            copy(line,requisitionLine,requisitionSchedule,s);
            s.setSnapshotLineId(snapshotId+":"+line.getLineNumber());
